...
pub fn main() {
    ...
    println!("One star result is {0}", one_star_result);
    let two_start_result = solve(&integers, 2020, 3);
    println!("Two star result is {0}", two_start_result);
}
...
fn solve(inputs: &[i32], sum_for: i32, addends: usize) -> i32 {
    find_result_recursive(inputs, sum_for, addends, 0, &[]).unwrap()
}
